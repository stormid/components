import {
	composeEcommerceExpressionParameters,
	exportEcommerceProductParameters,
	ACCEPTED_PARAMETERS,
...
const isValidCustomParam = param => CUSTOM_PARAMETERS[param.type.toUpperCase()]
			&& param.index
			&& param.value
...
export const event = data => Object.keys(data).reduce((acc, key) => {
...
export const linkEvent = (link, settings) => ({
    ...
    label: settings.obfuscator ? settings.obfuscator(link.href) : link.innerText,
    action: settings.obfuscator ? settings.obfuscator(link.href) : link.href
});

export const downloadEvent = (link, settings) => ({
	category: settings.category,
	label: settings.obfuscator ? settings.obfuscator(link.href) : link.href,
...
const composeImpressionList = (acc, curr, i) => {
	const parameters = composeEcommerceExpressionParameters(i + 1);
	const items = curr.items.reduce((acc, curr, j) => {
			const parameters = composeEcommerceExpressionParameters(i + 1, j + 1);
...
const composeProductList = (acc, curr, i) => {
	const parameters = exportEcommerceProductParameters(i + 1);
