...
const init = (selector, options) => {
	...
	if(nodes.length === 0) return console.warn(`Expand not initialised, no elements found for selector`, selector);

	//return array of objects, one for each DOM node found
	return nodes.map(btn => {
		return Object.create(factory({
			settings: { ...defaults, ...options, defaultLabel: btn.textContent },
			btn
		}));
	});
};
